...
void PrintNode(const Node&, std::ostream&, int, bool);
    ...
    void operator() (Array x) const {
        ...
        if (x.size() > 0)
        {
            ...
            for (const auto& item : x)
            {
                ...
                {
                    PrintTab();
                    if (item.IsArray() || item.IsMap())
                    {
                        PrintNode(item, out, depth + 1, true);
                    }
                    else
                    {
                        PrintNode(item, out, depth+1, false);
                    }
                    is_first = false;
                }
                else
        ...
        if (x.size() > 0)
        {
            ...
            for (auto it = x.begin(); it != x.end(); it++)
            {
                if (is_first)
                {
                    ...
                    PrintNode(it->second, out, depth + 1, true);
...
void PrintNode(const Node& node, std::ostream& out, int depth=0, bool flag=false) {
    visit(NodePrinter{out, depth, flag}, node.GetValue());
}
...
void Print(const Document& doc, ostream& output) {
    PrintNode(doc.GetRoot(), output, 0, false);
}
